...
const update = () => {
  /* Vuelve a dibujar los puntos, al pacman y
   los fantamas en su nueva posición. */
...
const interal = interval(1000).pipe(timeInterval());

let ghostSub = interal.subscribe(() => {
  moveGhost();
});
...
const checkDead = (position) => {
  if (
    position.pacMatrixX === ghostMatrixX &&
    position.pacMatrixY === ghostMatrixY
  ) {
